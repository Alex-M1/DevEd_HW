...
export default class View {
  ...
  raceStart = async (candidateList) => {
    ...
    const otherChecked = this.checkOtherDocuments(candidateList);
    await this.finalCheck(otherChecked);
  }

  finalCheck = async (otherChecked) => {
    try {
      const finish = await Promise.any(otherChecked);
      this.#drawText(`${finish[0].name} get visa!!!`, X * 34 - 40, 200);
    }
    catch (err) {
      this.#drawText('Nobody get visa', X * 34 - 40, 200);
    }
  }

  checkOtherDocuments = (candidateList) => {
    const otherChecked = [];
    candidateList.forEach((el, i) => {
      if (+el.balance >= 2000) {
